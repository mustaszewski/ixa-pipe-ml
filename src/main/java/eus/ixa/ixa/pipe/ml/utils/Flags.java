...
public class Flags {
	...
	public static final String DEFAULT_OUTCOMENGRAM_RANGE = "2";
	...
	public static String[] processSublabelRange(String sublabelRangeFlag) {
		...
		if (!sublabelRangeArray[1].equals("0")) {
			System.err.println(
					"The second field (after colon) of SublabelRange must be 0 but got " + sublabelRangeArray[1]);
			System.exit(1);
		}
	...
	public static String getPrevOutcomesFeaturesRange(TrainingParameters params) {
		String prevOutcomesRangeFlag = null;
		if (params.getSettings().get("PrevOutcomesRange") != null) {
		  prevOutcomesRangeFlag = params.getSettings().get("PrevOutcomesRange");
		  if (Integer.parseInt(prevOutcomesRangeFlag) < 1) {
			System.err.println("The Range of Previous Outcomes must be at least 1 but was set to " + prevOutcomesRangeFlag);
			System.exit(1);
		  }
		} else {
			prevOutcomesRangeFlag = Flags.DEFAULT_PREFIX_BEGIN;
		}
		return prevOutcomesRangeFlag;
	}

	public static String getPrevOutcomeNgramFeatures(TrainingParameters params) {
		String prevOutNgramFlag = null;
		if (params.getSettings().get("PrevOutcomeNgramFeatures") != null) {
			prevOutNgramFlag = params.getSettings().get("PrevOutcomeNgramFeatures");
		} else {
			prevOutNgramFlag = DEFAULT_FEATURE_FLAG;
		}
		return prevOutNgramFlag;
	}

	public static String getPrevOutcomeNgramRange(TrainingParameters params) {
		...
		if (params.getSettings().get("PrevOutcomeNgramRange") != null) {
			...
			if (Integer.parseInt(prevOutNgramRangeFlag) < 2) {
				System.err.println("The N-Gram range must be at least 2 be but was set to " + prevOutNgramRangeFlag);
				System.exit(1);
			}
		} else {
	...
	public static String getPrevOutcomeTokenFeatures(TrainingParameters params) {
		...
		if (params.getSettings().get("PrevOutcomeTokenFeature") != null) {
			...
			prevOutTokenFlag = DEFAULT_FEATURE_FLAG;
		}
	...
	public static String getPrevOutcomeTokenClassFeature(TrainingParameters params) {
		...
		if (params.getSettings().get("PrevOutcomeTokenClassFeature") != null) {
			...
			prevOutTokenClassFlag = DEFAULT_SENTENCE_END;
		}
	 ...
	 * clustering lexicons separated by comma "," and return a list of files,
	 * one for each lexicon.
	 * 
	 * @param clusterPath
	...
	public static boolean isWord2VecClusterFeatures(TrainingParameters params) {
		...
		return !word2vecClusterFeatures.equalsIgnoreCase(Flags.DEFAULT_FEATURE_FLAG);
	}
	...
	public static boolean isPrevOutcomesFeatures(TrainingParameters params) {
		String prevOutParam = getPrevOutcomesFeatures(params);
		return !prevOutParam.equalsIgnoreCase(Flags.DEFAULT_FEATURE_FLAG);
	}

	public static boolean isPrevOutcomeNgramFeatures(TrainingParameters params) {
		String prevOutParam = getPrevOutcomeNgramFeatures(params);
		return !prevOutParam.equalsIgnoreCase(Flags.DEFAULT_FEATURE_FLAG);
	}

	public static boolean isPrevOutcomeTokenFeatures(TrainingParameters params) {
		String prevOutParam = getPrevOutcomeTokenFeatures(params);
		return !prevOutParam.equalsIgnoreCase(Flags.DEFAULT_FEATURE_FLAG);
	}
